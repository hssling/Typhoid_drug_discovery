"""
Typhoid HDT v4.0 Global Impact Dashboard
Final version integrating AI, Pan-Enteric scaling, and Cloud-readiness.
"""
...
# HEADER
st.title("🌍 Enteric HDT Global Discovery Dashboard v4.0")
...
if df_targets is not None:
    if menu == "🗺️ Global Leaderboard":
        ...
        st.markdown("Using Deep Learning to predict ligand binding affinity based on molecular graphs.")
        ...
        if df_drugs is not None:
            ...
            with col2:
                fig = px.scatter(df_drugs, x='pChEMBL', y='AI_Predicted_pChEMBL', color='Max_Phase', hover_name='Drug_Name', title="AI Prediction Validation (True vs Predicted)")
        ...
        st.markdown("Identification of host targets effective against S. Typhi, S. Paratyphi A, and NTS.")
        
        fig = px.bar(df_targets.head(15), x='Symbol', y='Pan_Enteric_Conservation_Score', color='Scope', title="Pan-Enteric Conservation (Typhoid + Paratyphoid)")
        st.plotly_chart(fig, use_container_width=True)
        
        st.info("💡 **Clinical Insight**: Targets with high Pan-Enteric scores are ideal for broad-spectrum immunomodulation in endemic regions where mixed or poorly defined enteric fevers are common.")
        ...
        with col1:
            ...
            st.write(f"**Global Impact Score:** {target_row['Global_Impact_Score']}")
            st.write(f"**Causal Support:** {target_row['Causal_Evidence']}")
            ...
            values = [
                ...
                target_row['Structural_Fit_Score'] if 'Structural_Fit_Score' in target_row else 0.7,
                target_row['Cellular_Specificity_Score'],
                target_row['Genetic_Support_Score'],
                target_row['Pan_Enteric_Conservation_Score']
            ]
        ...
        with col2:
            ...
            if df_drugs is not None:
                ...
                if not gene_drugs.empty:
                    ...
                    st.warning("No high-affinity candidates found for this target.")

    elif menu == "📦 Reproducibility & Cloud":
        st.subheader("The Global Discovery Ecosystem")
        st.markdown("This project is designed for 100% reproducibility and public accessibility.")
        
        col1, col2 = st.columns(2)
        with col1:
            st.code("docker pull typhoid-hdt:v4.0", language="bash")
            st.markdown("- **Reproducibility**: Docker containerizes all dependencies (RDKit, Torch).")
            st.markdown("- **Transparency**: All CSV outputs and model weights are traceable.")
        
        with col2:
            st.markdown("- **Cloud-Ready**: Optimized for Streamlit Cloud deployment.")
            st.markdown("- **Open Science**: Ready for Zenodo/OSF archiving with DOI.")

else:
    st.error("Global data not found. Please run v4.0 and v5.0 scaling scripts.")

st.sidebar.markdown("---")
st.sidebar.info("Developed by Dr. Siddalingaiah H S | Enteric HDT Discovery v4.0")
